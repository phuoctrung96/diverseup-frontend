						...
						<div className={styles.leftPart}>
							...
							<div className={styles.socialLinks}>
								{socialLinks.map((link, index) => {
									return (
										<a key={index} href={link.link} className={`${styles.socialLink} ${styles[link.icon]}`} />
									);
								})}
							</div>
						</div>
							...
							<ul className={styles.footerMenu}>
								{
									footerLinks.map((item, index) => {
										return (
											<li key={index}>
												{!item?.subLinks?.length && <Link className={styles.title} to={item?.link || ''}>{ item.name }</Link>}
												{
															...
															<ul className={styles.footerSubMenu}>
																{
																	item.subLinks.map((subItem, subIndex) => {
																		return (
																			<li key={subIndex} className={subItem?.subClass ?`${styles.withIcon} ${styles[subItem?.subClass]}` : ''}>
																				{!subItem?.button && <a href={subItem?.link || ''}>{ subItem.name }</a>}
																				{subItem?.button && <Button text={subItem.name} onClick={subItem.onClick} classList={['footer']} />}
																			</li>
																		);
																	})
																}
